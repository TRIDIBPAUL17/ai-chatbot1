...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-50 bg-black/80 transition-opacity duration-300 data-[state=open]:fade-in data-[state=closed]:fade-out",
      className
    )}
    {...props}
    ref={ref}
...
const sheetVariants = cva(
  "fixed z-50 p-6 shadow-lg transition-transform duration-500 ease-in-out data-[state=open]:scale-100 data-[state=closed]:scale-95",
  {
    variants: {
      side: {
        ...
        right: "inset-y-0 right-0 w-3/4 border-l data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-sm",
      },
      size: {
        small: "max-w-sm",
        medium: "max-w-md",
...
const SheetContent = React.forwardRef<
  ...
  SheetContentProps
>(({ side = "right", size = "medium", animationDuration = "500ms", className, children, ...props }, ref) => (
  <SheetPortal>
    <SheetOverlay />
